...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if(lookDistance == -1)
	{
		...
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << " Você é " << (sex % 2 ? "um " : "uma ") << group->getName();
		else if(vocation_id != 0)
			s << " Você é " << (sex % 2 ? "um " : "uma ") << vocation->getDescription() << " de " << townss->getName().c_str();
		else
			s << " You have no vocation";
	}
	else
	{
		s << "Você está vendo " << nameDescription;
		if(!hasCustomFlag(PlayerCustomFlag_HideLevel))
			s << " (Level " << level << ")";

		s << ". " << (sex % 2 ? "Ele" : "Ela");
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << " é " << (sex % 2 ? "um " : "uma ") << group->getName();
		else if(vocation_id != 0)
			s << " é " << (sex % 2 ? "um " : "uma ") << vocation->getDescription();
		else
			s << " has no vocation";
