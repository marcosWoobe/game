...
class Creature : public AutoId, virtual public Thing
{
	...
	public:
		...
		virtual const int32_t& getStatValue(std::string stat) {return Stats[stat];}
		virtual const Monster* getMonster() const {return NULL;}
    	virtual const std::string& getNick() const {return Nick;}
    	virtual const uint8_t getPDuel() const {return pduel;}
    	virtual const int32_t getIDPad() const {return idpad;}
    	virtual const bool getGym() const {return gym;}
    	virtual const Position& getOrderPos() const {return orderPos;}
    	virtual const std::string& getOrderHabil() const {return orderHabil;}
    	virtual const std::string& getLastNick() const {return lastNick;}
    	virtual const std::string& getNickss() const {return "";}
    	virtual const std::string& getDitto() const {return ditto;}
		...
		virtual bool canWalkthrough(const Creature* creature) const {return creature->isWalkable() || creature->isGhost() || (this->isPlayerSummon() && this->getMaster() == creature);}
		...
		void setStatValue(std::string stat, int32_t value){Stats[stat] = value;}
		void setNick(std::string newNick);
		void setIDPad(int32_t idpoke) {idpad = idpoke;}
		void setPDuel(uint8_t duel) {pduel = duel;}
        void setGym(bool gyn) {gym = gyn;}
        void setOrderPos(const Position& pos) {orderPos = pos;}
        void setOrderHabil(std::string habi) {orderHabil = habi;}
		void setLastNick(std::string newNick) {lastNick = newNick;}
        ...
        int32_t getDistanceBetween() const;
		...
		int64_t getEventStepTicks() const; // Mixlort correção do Walk selvagem
		...
		//bool ehMonstro() const {return master && master >= 0x40000000 && master < 0x80000000;}
